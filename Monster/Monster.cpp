...
void AMonster::BeginPlay()
{
	...
	if (Info)
	{
		...
		{
			UMaterialInstanceDynamic* Mtrl = GetMesh()->CreateDynamicMaterialInstance(mDissolveMtrlIndexArray[i].Index, mDissolveMtrlIndexArray[i].Mtrl);
...
void AMonster::Tick(float DeltaTime)
{
	...
	UMonsterHPBase* HPWidget = Cast<UMonsterHPBase>(mWidgetComponent->GetWidget());	
	m_Ratio = (float)mInfo.HP / (float)mInfo.HPMax;
...
float AMonster::TakeDamage(float DamageAmount, FDamageEvent const& DamageEvent, AController* EventInstigator, AActor* DamageCauser)
{
	...
	//PrintViewport(5.f, FColor::Red, FString::Printf(TEXT(" %.5f"), (float)mInfo.HP));
